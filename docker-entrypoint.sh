...
if [ "$1" = 'nuxeoctl' ]; then
  if [ ! -f $NUXEO_HOME/configured ]; then
    ...
    mkdir -p ${NUXEO_DATA:=$NUXEO_DATA/var/lib/nuxeo/data}
    mkdir -p ${NUXEO_LOG:=$NUXEO_DATA/var/log/nuxeo}
    ...
    chown -R $NUXEO_USER:$NUXEO_USER $NUXEO_DATA/var/run/nuxeo

    cat << EOF >> $NUXEO_CONF
nuxeo.log.dir=$NUXEO_LOG
nuxeo.pid.dir=$NUXEO_DATA/var/run/nuxeo
