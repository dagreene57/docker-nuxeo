...
if [ "$1" = 'nuxeoctl' ]; then
  if [ ! -f $NUXEO_HOME/configured ]; then
    ...
    mkdir -p ${NUXEO_DATA:=$NUXEO_HOME/var/lib/nuxeo/data}
    mkdir -p ${NUXEO_LOG:=$NUXEO_HOME/var/log/nuxeo}
    mkdir -p $NUXEO_HOME/var/run/nuxeo
    ...
    chown -R $NUXEO_USER:$NUXEO_USER $NUXEO_HOME/var/run/nuxeo

    cat << EOF >> $NUXEO_CONF
nuxeo.log.dir=$NUXEO_LOG
nuxeo.pid.dir=$NUXEO_HOME/var/run/nuxeo
